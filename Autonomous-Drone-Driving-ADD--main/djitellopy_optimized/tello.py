...
class Tello:
    ...
    TIME_BTW_COMMANDS = 0.05 # in seconds
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> str:
        ...
        if diff < self.TIME_BTW_COMMANDS:
            ...
            time.sleep(self.TIME_BTW_COMMANDS - diff) #Dors juste pour le temps nécessaire
        ...
        while not responses:
            if time.time() - timestamp > timeout:
                ...
                return message
            time.sleep(0.05)  # Sleep during send command réduit de 0.1 à 0.05
    ...
    #On remplace les deux commandes rotates par une unique
    def rotate(self, degrees: int):
        if degrees > 0:
            self.send_control_command("cw {}".format(degrees))
        elif degrees < 0:
            self.send_control_command("ccw {}".format(-degrees))
